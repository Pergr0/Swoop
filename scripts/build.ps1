...
function Ensure-Wails {
    ...
    if (-not (Test-Cmd go)) { Write-Fail "Go is required to install the Wails CLI"; exit 1 }
...
function Sync-AppIcon {
    ...
    try {
        ...
        if ($LASTEXITCODE -ne 0) {
            throw "failed to regenerate icon.ico from build\appicon.png"
        }
    } finally {
        Pop-Location
...
function Invoke-Build {
    ...
    try {
        ...
        Ensure-FrontendDeps
        Build-Frontend
        Write-Info "running: wails build -s -f"
        & wails build -s -f
        $code = $LASTEXITCODE
        if ($code -eq 0) {
